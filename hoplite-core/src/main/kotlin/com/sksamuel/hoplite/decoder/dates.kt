...
class LocalDateTimeDecoder : NonNullableLeafDecoder<LocalDateTime> {
  ...
  ): ConfigResult<LocalDateTime> = when (node) {
...
class DurationDecoder : NonNullableLeafDecoder<Duration> {
  ...
  override fun safeLeafDecode(
    ...
    type: KType,
    context: DecoderContext
  ): ConfigResult<Duration> = when (node) {
    is StringNode -> parseDuration(node.value).flatRecover {
...
class KotlinDurationDecoder : NonNullableLeafDecoder<kotlin.time.Duration> {
  ...
  override fun safeLeafDecode(
    ...
    type: KType,
    context: DecoderContext
  ): ConfigResult<kotlin.time.Duration> = when (node) {
    is StringNode -> parseDuration(node.value).map { it.toMillis().milliseconds }
      .mapInvalid { ConfigFailure.DecodeError(node, type) }
...
class InstantDecoder : NonNullableLeafDecoder<Instant> {
  ...
  override fun safeLeafDecode(
    ...
    is StringNode -> runCatching { Instant.ofEpochMilli(node.value.toLong()) }.recoverCatching { Instant.parse(node.value) }
      .toValidated {
        ConfigFailure.DecodeError(node, type)
      }
...
class YearMonthDecoder : NonNullableLeafDecoder<YearMonth> {
  ...
  override fun safeLeafDecode(
    ...
    type: KType,
    context: DecoderContext
  ): ConfigResult<YearMonth> = when (node) {
...
class MonthDayDecoder : NonNullableLeafDecoder<MonthDay> {
  ...
  override fun safeLeafDecode(
    ...
    type: KType,
    context: DecoderContext
  ): ConfigResult<MonthDay> = when (node) {
...
class SqlTimestampDecoder : NonNullableLeafDecoder<java.sql.Timestamp> {
  ...
  override fun safeLeafDecode(
    ...
    type: KType,
    context: DecoderContext
  ): ConfigResult<java.sql.Timestamp> = when (node) {
